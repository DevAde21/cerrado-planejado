       ...
       if (hasCursor()) {
            ...
            window.removeEventListener('mouseleave', endDrag);
             ...
             productsNextButton.addEventListener('click', () => {
                if (productsIsTransitioning) return;
                    if (productsFirstVisibleItem >= productsTotalItems - productsItemsToShow - 1) {
                    productsCarouselTrack.style.transition = 'none';
                    productsFirstVisibleItem = productsItemsToShow - 1;
                    productsCarouselTrack.style.transform = `translateX(${calculateProductsSlidePosition(productsFirstVisibleItem)}px)`;
                    productsCarouselTrack.offsetHeight;
                }
                    moveToProductsSlide(productsFirstVisibleItem + 1);
                });
        ...
        if (Math.abs(diff) > productsThreshold) {
            ...
            if(diff > 0) { //Dragging right
                 if (targetIndex < productsItemsToShow) {
                    productsCarouselTrack.style.transition = 'none';
                    targetIndex = productsTotalItems - productsItemsToShow + (targetIndex - productsItemsToShow);
                    productsCarouselTrack.style.transform = `translateX(${calculateProductsSlidePosition(targetIndex)}px)`;
                   productsCarouselTrack.offsetHeight;
                 }
            }else { //Dragging left
                if (targetIndex > productsTotalItems - productsItemsToShow -1) {
                     productsCarouselTrack.style.transition = 'none';
                     targetIndex = productsItemsToShow + (targetIndex - (productsTotalItems - productsItemsToShow));
                     productsCarouselTrack.style.transform = `translateX(${calculateProductsSlidePosition(targetIndex)}px)`;
                    productsCarouselTrack.offsetHeight;
                }
            }
             moveToProductsSlide(targetIndex);
        } else {
            productsCarouselTrack.style.transition = 'transform 0.2s ease-in-out';
            productsCarouselTrack.style.transform = `translateX(${calculateProductsSlidePosition(productsFirstVisibleItem)}px)`;
        }
